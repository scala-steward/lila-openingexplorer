...
impl RawUciMove {
    pub fn pack(uci: UciMove) -> RawUciMove {
    ...
    pub fn unpack(self) -> UciMove {
        let from = Square::new(u32::from(self.0 & 63));
        let to = Square::new(u32::from((self.0 >> 6) & 63));
        let role = Role::try_from(self.0 >> 12).ok();
        if from == to {
            match role {
...
impl fmt::Debug for RawUciMove {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "RawUciMove({})", self.unpack())
    }
}
...
mod tests {
    ...
    fn test_uci_move_roundtrip() {
        ...
        let mut buf = Vec::new();
        for uci in moves {
            RawUciMove::pack(uci).write(&mut buf);
        }

        let mut reader = &buf[..];
        for uci in moves {
            assert_eq!(uci, RawUciMove::read(&mut reader).unpack());
        }
    }
}
